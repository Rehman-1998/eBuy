...
function MeetingView({ onNewMeetingIdToken, onMeetingLeave }) {
  ...
  if (meetingId) {
    console.log("Meeting View =====>", meetingId);
  ...
  return (
    ...
    >
      <div className="my-4">
        <button
          className={"button red mx-2 my-2"}
          onClick={() => {
            dispatch(settingMeetingId("")), leave();
          }}
        >
          Leave Meeting
        </button>
        <button className={"button blue mx-2 my-2"} onClick={toggleMic}>
          toggleMic
        </button>
        <button
          className={"button blue mx-2 my-2"}
          onClick={() => {
            toggleWebcam();
          }}
        >
...
const App = () => {
  ...
  const [meetingId, setMeetingId] = useState(
    location?.state?.meetingId ? location?.state?.meetingId : ""
  );
    ...
    >
      <MeetingView
        onNewMeetingIdToken={({ meetingId, token }) => {
          console.log("IN Meeting View", meetingId);
