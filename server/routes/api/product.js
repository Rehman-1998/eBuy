...
router.post("/set-meeting-id", async (req, res) => {
  try {
    const { meetingId, merchant } = req.body;
    const getProducts = await Product.updateMany(
      { merchant },
      { meetingId },
      { new: true }
    );
    console.log("Req=====>", req.body, getProducts);
    if (getProducts) {
      res.status(200).json({
        message: "Your Meeting ID updated Successfully",
        data: getProducts,
        success: true,
      });
    }
